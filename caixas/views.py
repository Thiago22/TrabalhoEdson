...
def caixaFluxo(request):
    if request.method == 'POST':
        ...
        data_inicio = datetime.strptime(request.POST.get('data_inicio', ''), '%d/%m/%Y %H:%M:%S') #1º DATA
        ...
        return render(request, 'caixas/formFluxo_Caixa.html', {'contas' : contas, 'total': total ,'data_inicio': data_inicio, 'data_final': data_final})

    return render(request, 'caixas/formFluxo_Caixa.html', {'contas' : []})
